...
func (s *SpecFactory) GetWorkerDeploymentSpec(w *WorkerInfo) *appsv1.Deployment {
	...
	container := corev1.Container{
		Name:            "mjs-worker",
		Image:           s.config.MatlabImage,
		ImagePullPolicy: corev1.PullPolicy(s.config.MatlabImagePullPolicy),
...
func (s *SpecFactory) GetJobManagerDeploymentSpec() *appsv1.Deployment {
	...
	// Mount custom phoenix config file
	if s.config.OverrideWrapperPhoenix {
		const wrapperPhoenixFile = "wrapper-phoenix.config"
		configPath := filepath.Join(s.config.MatlabRoot, "toolbox", "parallel", "config", wrapperPhoenixFile)
		addVolumeFromConfigMapFile(&pod, "mjs-phoenix-config", "phoenix-volume", wrapperPhoenixFile, configPath)
	}
