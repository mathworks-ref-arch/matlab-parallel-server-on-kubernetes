...
func verifyWorkerSpecs(t *testing.T, specFactory *SpecFactory, conf *config.Config, ownerUID types.UID) {
	...
	assert.Equal(conf.MatlabImage, container.Image, "Worker container has incorrect image")
	assert.Equal(conf.MatlabImagePullPolicy, string(container.ImagePullPolicy), "Worker container has incorrect image pull policy")
...
func verifyJobManagerSpec(t *testing.T, specFactory *SpecFactory, conf *config.Config, ownerUID types.UID) {
	...
	container := pod.Containers[0]
	assert.Equal(conf.MatlabImage, container.Image, "Job manager container has incorrect image")
	assert.Equal(conf.MatlabImagePullPolicy, string(container.ImagePullPolicy), "Job manager container has incorrect image pull policy")
...
func createTestConfig() *config.Config {
	return &config.Config{
		...
		MatlabImagePullPolicy:    "Never",
		MatlabImage:              "my-matlab-image",
		LogBase:                  "/var/logs",
		LogLevel:                 3,
